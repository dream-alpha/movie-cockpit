...
class MovieCoverDownload(MovieTMDB, MovieCover):
	...
	def downloadCover(self, cover_url, cover_path, backdrop_url=None):
		...
		logger.info("backdrop_url: %s", backdrop_url)
		cover_found = 0
		if cover_url is not None:
			deleteFile(cover_path)
			logger.debug("cover_path: %s, cover_url: %s", cover_path, cover_url)
			rc = os.popen("wget -qO \"" + cover_path + "\" " + cover_url).read()
			cover_found = rc == 0
			if rc:
				logger.error("cover: rc: %s", rc)
		...
		if backdrop_url is not None:
			...
			logger.debug("backdrop_path: %s, backdrop_url: %s", backdrop_path, backdrop_url)
			rc = os.popen("wget -qO \"" + backdrop_path + "\" " + backdrop_url).read()
			if rc:
				logger.error("backdrop: rc: %s", rc)
		return cover_found
