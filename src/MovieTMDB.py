...
class MovieTMDB():
	...
	def getMovieList(self, moviename):
		...
		def getMovies(moviename):
			response = self.fetchData("http://api.themoviedb.org/3/search/movie?api_key=3b6703b8734fee1b598de9ed7bbd3b47&query=" + moviename.replace(" ", "+").replace("&", "%26"))
			movielist = []
			if response:
		...
		def getTvShows(moviename):
			response = self.fetchData("http://api.themoviedb.org/3/search/tv?api_key=3b6703b8734fee1b598de9ed7bbd3b47&query=" + moviename.replace(" ", "+").replace("&", "%26"))
			tvshowslist = []
			if response:
